...
namespace Kiosk.db_lite
{
    class DBRestaurant : DBModel
    {
        ...
        public List<Restaurant> getRestaurants()
        {
            ...
            SQLiteDataReader dataReader = db.select("select * from restaurants");
            if (dataReader != null)
            {
        ...
        public List<Printer> getPrinters(Restaurant rest)
        {
            ...
            values.Clear();
            values.Add("@id", rest.id.ToString());
            SQLiteDataReader dataReader = db.select("select * from printers where restaurant_id=@id", values);
            if (dataReader != null)
            {
                while (dataReader.Read())
                {
        ...
        public Restaurant getRestaurant(int id)
        {
            ...
            SQLiteDataReader dataReader = db.select("select * from restaurants where id=@id limit 1", values);
            if (dataReader != null)
            {
                while (dataReader.Read())
                {
        ...
        public Discount getDiscount(Restaurant rest, string code)
        {
            ...
            values.Add("@now", now);
            SQLiteDataReader dataReader = db.select("select * from discounts where restaurant_id=@restaurant_id and code like @code and @now >= started_at and @now <= invoked_at and count > 0 limit 1", values);
            if (dataReader != null)
            {
                while (dataReader.Read())
                {
